...
class ListsSuite {
  ...
  @Test def `max of negative -3 to 0 is 0`: Unit = {
    Assert.assertEquals(0, max(List(-2, -3, 0, -1)))
  }

  @Test def `sum of negative -3 to 0 is -6`: Unit = {
    Assert.assertEquals(-6, sum(List(-3, -2, -1, 0)))
  }

  @Test def `in a sum of empyt list is equal to zero`: Unit = {
    Assert.assertEquals(0, sum(List()))
  }

  @Test def `in max nost alowed empty List`: Unit={
    try{
      max(List())
      Assert.fail("Should thorw NoSuchElementException ");
    } catch {
